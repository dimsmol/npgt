...
Db.prototype.fetchAll = function (options, cb) {
	...
	var ctx = this.createCtx(query, data);
	var rowHandler = function (row) {
		if (!ctx.hasError()) {
			if (transform != null) {
				row = transform(row, ctx);
			}
			if (collect != null) {
				collect(row, ctx.data, ctx);
			}
			else {
				ctx.data.push(row);
			}
...
Db.prototype.createCtx = function (query, opt_initialData) {
	return new Ctx(query, opt_initialData);
};
