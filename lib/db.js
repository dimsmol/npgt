...
Db.prototype.execute = function (pg, config, func, cb) {
	// compatibility
	if (cb == null && pg != null && (pg.connect == null || pg.connect.constructor !== Function)) {
		cb = func;
		func = config;
		config = pg;
		pg = pgLib;
...
Db.prototype.transaction = function (client, f, cb) {
	new Transaction(this, client, f, cb).execute();
};

Db.prototype.begin = function (client, cb) {
	this.nonQueryVoid(client.query('begin'), cb);
};

Db.prototype.commit = function (client, cb) {
	this.nonQueryVoid(client.query('commit'), cb);
};

Db.prototype.rollback = function (client, cb) {
	this.nonQueryVoid(client.query('rollback'), cb);
};

Db.prototype.savepoint = function (client, savepoint, cb) {
	this.nonQueryVoid(client.query('savepoint ' + savepoint), cb);
};

Db.prototype.rollbackTo = function (client, savepoint, cb) {
	this.nonQueryVoid(client.query('rollback to ' + savepoint), cb);
};
...
Db.prototype.nonQueryVoid = function (query, cb) {
	this.executeQuery(query, null, function (err) {
		cb(err);
	});
};
